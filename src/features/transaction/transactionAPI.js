...
export async function createTransaction(
  ...
  const transaction = await axios(
    "hhttps://nkwsv.kryptwallet.com/api/transactions",
    {
      method: "POST",
      headers: {
...
export async function getWalletTx(walletId, token) {
  ...
  const tx = await fetch(
    `https://nkwsv.kryptwallet.com/api/transactions/%23${address}`,
    {
      method: "GET",
      headers: {
        "Content-Type": "application/json",
...
export async function pkTransaction({ walletId, amount }, token) {
  const transaction = await axios(
    "https://nkwsv.kryptwallet.com/api/transactions/requestpk",
    {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
